...
class CacheEnv(object):
    ...
    def step(self, action, request_dataset, v2i_rate, v2i_rate_mbs, vehicle_epoch, vehicle_request_num, print_step):
        ...
        if action == 1:
            # 确定要替换的内容数量，最多5个
            num_replace = min(5, len(self.last_content))
            # 从 last_content 中随机选取要替换的内容
            replace_content = random.sample(self.last_content, num_replace)
            # 保存将被替换出去的内容
            replaced_out_content = self.state[-num_replace:]
            # 执行替换操作
            for idx, item in enumerate(replace_content):
                self.state[-(idx + 1)] = item

            # 从 last_content 中移除已被选入 state 的内容
            for item in replace_content:
                self.last_content.remove(item)
            # 将被替换出去的内容加入 last_content
            self.last_content.extend(replaced_out_content)

            # 根据流行度重新排序 state
            self.state = [item for item in self.popular_content if item in self.state]

            # 更新 state2，确保内容与 state 和 last_content 不重叠，且按流行度排序
            self.state2 = [item for item in self.popular_content if item not in self.state and item not in self.last_content]
            # 截取 state2 的大小至 cache_size
            self.state2 = self.state2[:self.cache_size]

            # 确保 last_content 不包含 state 和 state2 的内容
            self.last_content = [item for item in self.last_content if item not in self.state2]
