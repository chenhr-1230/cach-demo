...
class MovieDataset(Dataset):
    ...
    def __init__(self, dataset, idxs):
        ...
        self.idxs = [int(i) for i in idxs] #车辆vec的请求数据索引列表
    ...
    def __getitem__(self, item):
        ratings = self.dataset[item, 1:-3]  # 直接索引self.dataset中的第item行
        return torch.tensor(ratings)
...
class LocalUpdate(object):
    def __init__(self, args, dataset, idxs):
        ...
        self.dataset = convert(dataset[idxs], int(max(dataset[:, 1]))) #dataset = userid , movie max个rating ,gender  age  occupation. 某一辆车的请求的数据
        self.idxs = np.arange(0, len(self.dataset)) #users_group_train[veh]
...
def cache_hit_ratio_compare(test_dataset, cache_items):
    """
    计算缓存命中率
    :param test_dataset: user_group_test[0-9] dataset
    :param cache_items: 缓存内容
